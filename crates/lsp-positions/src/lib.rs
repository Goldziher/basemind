...
//! Maintained fork of `lsp-positions` (origin repo <https://github.com/Goldziher/basemind>),
//! originally derived from github/stack-graphs lsp-positions v0.3.4 and modernized (tree-sitter
//! 0.26, edition 2024). See `ATTRIBUTIONS.md` for provenance.
//!
...
impl<'a> SpanCalculator<'a> {
    ...
    fn replace_current_line(&mut self, line_utf8_offset: usize) {
        if let Some(containing_line) = &self.containing_line
            && containing_line.utf8_bounds.start == line_utf8_offset
        {
            return;
        }
