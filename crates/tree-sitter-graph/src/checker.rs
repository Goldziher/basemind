    ...
    pub fn check(&mut self) -> Result<(), CheckError> {
        ...
        for global in &self.globals {
            globals
                ...
                .map_err(|_| CheckError::DuplicateGlobalVariable(global.name.as_str().to_string(), global.location))?;
        }
...
impl ast::Stanza {
    fn check(
        ...
        if !unused_captures.is_empty() {
            return Err(CheckError::UnusedCaptures(unused_captures.join(" "), self.range.start));
        }
...
impl ast::ForIn {
    fn check(&mut self, ctx: &mut CheckContext) -> Result<StatementResult, CheckError> {
        ...
        let var_result = self.variable.check_add(&mut loop_ctx, value_result.into(), false)?;
        used_captures.extend(var_result.used_captures);
    ...
    fn check(&mut self, ctx: &mut CheckContext) -> Result<ExpressionResult, CheckError> {
        ...
        self.stanza_capture_index =
            ctx.stanza_query
                ...
                .ok_or_else(|| CheckError::UndefinedSyntaxCapture(name.clone(), self.location))? as usize;
...
impl ast::Variable {
    ...
    fn check_set(&mut self, ctx: &mut CheckContext, value: VariableResult) -> Result<StatementResult, CheckError> {
        match self {
...
impl ast::ScopedVariable {
    ...
    fn check_set(&mut self, ctx: &mut CheckContext, _value: VariableResult) -> Result<StatementResult, CheckError> {
        let scope_result = self.scope.check(ctx)?;
        Ok(scope_result.into())
    }
