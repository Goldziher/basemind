...
impl File {
    ...
    fn check_globals(&self, globals: &mut Globals) -> Result<(), ExecutionError> {
            ...
            match globals.get(&global.name) {
                None => {
                    if let Some(default) = &global.default {
                        ...
                        return Err(ExecutionError::MissingGlobalVariable(global.name.as_str().to_string()));
                    }
                }
                Some(value) => {
                    ...
                    {
                        return Err(ExecutionError::ExpectedList(global.name.as_str().to_string()));
                    }
                }
            }
        }
            ...
            self.try_visit_matches_lazy(tree, source, |stanza, mat| {
                let named_captures = stanza
                    ...
                    .map(|name| {
                        ...
                        let quantifier = file_query.capture_quantifiers(mat.pattern_index)[index as usize];
...
impl Stanza {
    pub fn try_visit_matches<'tree, E, F>(&self, tree: &'tree Tree, source: &'tree str, mut visit: F) -> Result<(), E>
    where
        F: FnMut(Match<'_, 'tree>) -> Result<(), E>,
    {
    ...
    pub fn from_nodes<'tree, NI: IntoIterator<Item = Node<'tree>>>(
        ...
        match quantifier {
            ...
            CaptureQuantifier::ZeroOrMore | CaptureQuantifier::OneOrMore => {
                let syntax_nodes = nodes.map(|n| graph.add_syntax_node(n).into()).collect::<Vec<Value>>();
                syntax_nodes.into()
            }
    ...
    pub(crate) fn add_debug_attrs(
        ...
        if let Some(location_attr) = &config.location_attr {
            attributes
                .add(
                    location_attr.clone(),
                    format!("line {} column {}", self.location.row + 1, self.location.column + 1),
                )
    ...
    pub(crate) fn add_debug_attrs(
        ...
        if let Some(variable_name_attr) = &config.variable_name_attr {
            attributes
                ...
                .map_err(|_| ExecutionError::DuplicateAttribute(variable_name_attr.as_str().into()))?;
        }
        if let Some(location_attr) = &config.location_attr {
