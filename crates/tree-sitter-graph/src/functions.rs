...
pub trait Function {
    fn call(&self, graph: &mut Graph, source: &str, parameters: &mut dyn Parameters) -> Result<Value, ExecutionError>;
}
    ...
    I: Iterator<Item = Value>,
{
    fn param(&mut self) -> Result<Value, ExecutionError> {
        let value = self.next().ok_or(ExecutionError::InvalidParameters(
            "expected more parameters".to_string(),
        ))?;
        Ok(value)
    }
...
impl Functions {
    ...
    pub fn stdlib() -> Functions {
        ...
        functions.add(Identifier::from("start-column"), stdlib::syntax::StartColumn);
            ...
            fn call(
                ...
                let parent = match node.parent() {
                    ...
                    None => {
                        return Err(ExecutionError::FunctionFailed(
                            ...
                            "Cannot call named-child-index on the root node".to_string(),
                        ));
                    }
                };
                ...
                let index = parent
                    ...
                    .ok_or(ExecutionError::FunctionFailed(
                        "named-child-index".into(),
                        "Called named-child-index on a non-named child".to_string(),
                    ))?;
                        ...
                        '{' => match it.next() {
                            ...
                            Some((i, c)) => {
                                return Err(ExecutionError::FunctionFailed(
                                    "format".into(),
                                    format!(
                                        "Unexpected character `{}` after `{{` at position {} in format string `{}`. Expected `{{` or `}}`.",
                                        c,
                                        i + 1,
                                        format
                                    ),
                            ...
                            None => {
                                return Err(ExecutionError::FunctionFailed(
                                    "format".into(),
                                    format!(
                                        "Unexpected end of format string `{}` after `{{`. Expected `{{` or `}}`.",
                                        format
                                    ),
                                ));
                            }
                        ...
                        '}' => match it.next() {
                            ...
                            Some((i, c)) => {
                                return Err(ExecutionError::FunctionFailed(
                                    "format".into(),
                                    format!(
                                        "Unexpected character `{}` after `}}` at position {} in format string `{}`. Expected `}}`.",
                                        c,
                                        i + 1,
                                        format
                                    ),
                            ...
                            None => {
                                return Err(ExecutionError::FunctionFailed(
                                    "format".into(),
                                    format!("Unexpected end of format string `{}` after `{{. Expected `}}`.", format),
                                ));
                            }
                        },
                        c => result.push(c),
        ...
        impl Function for Replace {
            fn call(
                ...
                let pattern = Regex::new(&pattern)
                    .map_err(|e| ExecutionError::FunctionFailed("replace".into(), format!("{}", e)))?;
