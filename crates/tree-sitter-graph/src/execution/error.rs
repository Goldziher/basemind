...
impl DisplayExecutionErrorPretty<'_> {
    fn fmt_entry(&self, f: &mut std::fmt::Formatter<'_>, index: usize, error: &ExecutionError) -> std::fmt::Result {
        match error {
            ExecutionError::InContext(context, cause) => {
                match context {
                    Context::Statement(stmts) => {
                        let mut first = true;
                        for stmt in stmts {
                            stmt.fmt_pretty(f, self.source_path, self.source, self.tsg_path, self.tsg, index, first)?;
                            first = false;
                        }
                    }
...
impl StatementContext {
    ...
    fn fmt_pretty(
        ...
        if first {
            writeln!(f, "{:>5}: Error executing statement {}", index, self.statement)?;
        } else {
