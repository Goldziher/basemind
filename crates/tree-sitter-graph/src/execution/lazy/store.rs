...
impl LazyVariable {
    ...
    pub(super) fn evaluate(&self, exec: &mut EvaluationContext) -> Result<graph::Value, ExecutionError> {
...
impl LazyScopedVariables {
    ...
    pub(super) fn add(
        ...
        match values.replace(ScopedValues::Forcing) {
            ...
            ScopedValues::Forcing => Err(ExecutionError::RecursivelyDefinedScopedVariable(format!("{}", name))),
            ScopedValues::Forced(map) => {
                values.replace(ScopedValues::Forced(map));
                Err(ExecutionError::VariableScopesAlreadyForced(format!("{}", name)))
            }
        }
    }
    ...
    pub(super) fn evaluate(
        ...
        let cell = match self.variables.get(name) {
            Some(v) => v,
            None => {
                return Err(ExecutionError::UndefinedScopedVariable(format!("{}.{}", scope, name,)));
            }
        };
    ...
    fn force(
        ...
        match values {
                ...
                for (scope, value, debug_info) in pairs.into_iter() {
                    ...
                    match (
                        ...
                        (Some(_), Some(prev_debug_info)) => {
                            return Err(ExecutionError::DuplicateVariable(format!("{}.{}", node, name,)))
                                .with_context(|| (prev_debug_info.0, debug_info.0).into());
                        }
                        (Some(_), None) => {
                            unreachable!("previous value for syntax node {} without previous debug info", node)
                        }
                        _ => {}
                    };
                }
            ...
            ScopedValues::Forcing => Err(ExecutionError::RecursivelyDefinedScopedVariable(format!("_.{}", name))),
