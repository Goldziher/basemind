...
impl LazyStatement {
    pub(super) fn evaluate(&self, exec: &mut EvaluationContext) -> Result<(), ExecutionError> {
        ...
        let result = match self {
            Self::AddGraphNodeAttribute(stmt) => stmt.evaluate(exec).with_context(|| stmt.debug_info.clone().into()),
            Self::CreateEdge(stmt) => stmt.evaluate(exec).with_context(|| stmt.debug_info.clone().into()),
            Self::AddEdgeAttribute(stmt) => stmt.evaluate(exec).with_context(|| stmt.debug_info.clone().into()),
            Self::Print(stmt) => stmt.evaluate(exec).with_context(|| stmt.debug_info.clone().into()),
        };
        trace!("}}");
        result
    }
...
impl LazyAddGraphNodeAttribute {
    pub(super) fn new(node: LazyValue, attributes: Vec<LazyAttribute>, debug_info: DebugInfo) -> Self {
        Self {
            node,
            attributes,
            debug_info,
    ...
    pub(super) fn evaluate(&self, exec: &mut EvaluationContext) -> Result<(), ExecutionError> {
        ...
        for attribute in &self.attributes {
            ...
            if exec.graph[node].attributes.add(attribute.name.clone(), value).is_err() {
                return Err(ExecutionError::DuplicateAttribute(format!(
                    "{} on {}",
                    attribute.name, node,
                )))
                .with_context(|| (prev_debug_info.unwrap().into(), self.debug_info.clone().into()).into());
            };
        }
        Ok(())
    }
...
impl LazyCreateEdge {
    pub(super) fn new(source: LazyValue, sink: LazyValue, attributes: Attributes, debug_info: DebugInfo) -> Self {
        Self {
            source,
            sink,
            attributes,
...
impl fmt::Display for LazyCreateEdge {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "edge {} -> {} at {}", self.source, self.sink, self.debug_info,)
    }
}
