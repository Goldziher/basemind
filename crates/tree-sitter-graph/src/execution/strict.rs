...
impl AddGraphNodeAttribute {
    fn execute(&self, exec: &mut ExecutionContext) -> Result<(), ExecutionError> {
        ...
        let add_attribute = |exec: &mut ExecutionContext, name: Identifier, value: Value| {
            exec.graph[node].attributes.add(name.clone(), value).map_err(|_| {
                ExecutionError::DuplicateAttribute(format!(" {} on graph node ({}) in {}", name, node, self,))
            })
        };
    ...
    fn execute(&self, exec: &mut ExecutionContext) -> Result<(), ExecutionError> {
        ...
        let add_attribute = |exec: &mut ExecutionContext, name: Identifier, value: Value| {
            ...
            edge.attributes.add(name.clone(), value).map_err(|_| {
                ExecutionError::DuplicateAttribute(format!(" {} on edge ({} -> {}) in {}", name, source, sink, self,))
            })
        };
        for attribute in &self.attributes {
            attribute.execute(exec, &add_attribute)?;
...
impl Capture {
    fn evaluate(&self, exec: &mut ExecutionContext) -> Result<Value, ExecutionError> {
        Ok(Value::from_nodes(
            exec.graph,
            exec.mat.nodes_for_capture_index(self.stanza_capture_index as u32),
            self.quantifier,
        ))
    }
}
...
impl Variable {
    ...
    fn add(&self, exec: &mut ExecutionContext, value: Value, mutable: bool) -> Result<(), ExecutionError> {
        match self {
    ...
    fn get<'a>(&self, exec: &'a mut ExecutionContext) -> Result<&'a Value, ExecutionError> {
        ...
        if exec.inherited_variables.contains(&self.name) {
            ...
            while let Some(scope) = parent {
                if let Some(value) = exec.scoped.try_get(scope.id() as u32).and_then(|v| v.get(&self.name)) {
                    return Ok(value);
                }
                parent = scope.parent();
            }
        ...
        Err(ExecutionError::UndefinedVariable(format!("{} on node {}", self, scope)))
    }
...
impl Attribute {
    fn execute<F>(&self, exec: &mut ExecutionContext, add_attribute: &F) -> Result<(), ExecutionError>
    where
...
impl AttributeShorthand {
    fn execute<F>(&self, exec: &mut ExecutionContext, add_attribute: &F, value: Value) -> Result<(), ExecutionError>
    where
        F: Fn(&mut ExecutionContext, Identifier, Value) -> Result<(), ExecutionError>,
    {
