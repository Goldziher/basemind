...
impl std::fmt::Display for DisplayParseErrorPretty<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(
            f,
            "{}",
            Excerpt::from_source(self.path, self.source, location.row, location.to_column_range(), 0)
        )?;
        Ok(())
    }
}
...
impl<'a> Parser<'a> {
    ...
    fn next(&mut self) -> Result<char, ParseError> {
        let ch = self.chars.next().ok_or(ParseError::UnexpectedEOF(self.location))?;
    ...
    fn parse_statement(&mut self) -> Result<ast::Statement, ParseError> {
        ...
        if keyword == "let" {
            ...
            while self.peek()? != '}' {
                ...
                let regex = Regex::new(&pattern).map_err(|_| ParseError::InvalidRegex(pattern, pattern_location))?;
                self.consume_whitespace();
                let statements = self.parse_statements()?;
            ...
            Err(ParseError::UnexpectedKeyword(keyword.into(), keyword_location))
        }
    }
    ...
    fn parse_condition(&mut self) -> Result<ast::Condition, ParseError> {
        ...
        let condition = if self.consume_token("some").is_ok() {
            ...
            return Err(ParseError::ExpectedToken("(some|none)? EXPRESSION", location));
        };
        self.consume_whitespace();
        Ok(condition)
    }
    ...
    fn parse_expression(&mut self) -> Result<ast::Expression, ParseError> {
        let mut expression = match self.peek()? {
            ...
            ch => return Err(ParseError::UnexpectedCharacter(ch, "expression", self.location)),
        };
        self.consume_whitespace();
        while self.try_peek() == Some('.') {
            self.skip().unwrap();
            ...
            self.consume_whitespace();
            expression = ast::ScopedVariable { scope, name, location }.into();
        }
        Ok(expression)
    }
    ...
    fn parse_call(&mut self) -> Result<ast::Expression, ParseError> {
        ...
        self.consume_token(")")?;
        Ok(ast::Call { function, parameters }.into())
    }
    ...
    fn parse_capture(&mut self) -> Result<ast::Capture, ParseError> {
        ...
        let ch = self.next()?;
        if !is_ident_start(ch) {
            return Err(ParseError::UnexpectedCharacter(ch, "query capture", self.location));
        }
        self.consume_while(is_ident);
        let end = self.offset;
    ...
    fn parse_literal(&mut self) -> Result<ast::Expression, ParseError> {
        ...
        if literal == "false" {
            ...
            Ok(ast::Expression::NullLiteral)
        } else if literal == "true" {
            Ok(ast::Expression::TrueLiteral)
        } else {
            Err(ParseError::UnexpectedLiteral(literal.into(), literal_location))
        }
    }
    ...
    fn parse_unscoped_variable(&mut self) -> Result<ast::UnscopedVariable, ParseError> {
        match self.parse_variable()? {
            ...
            ast::Variable::Scoped(variable) => Err(ParseError::ExpectedUnscopedVariable(variable.location)),
        }
    }
}
