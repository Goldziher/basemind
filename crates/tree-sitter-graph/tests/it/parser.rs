...
fn can_parse_blocks() {
    ...
    "#;
    let file = File::from_str(tree_sitter_python::LANGUAGE.into(), source).expect("Cannot parse file");
    ...
    let statements = file.stanzas.into_iter().map(|s| s.statements).collect::<Vec<_>>();
    assert_eq!(
        statements,
        vec![vec![
            CreateGraphNode {
                ...
                variable: ScopedVariable {
                    scope: Box::new(
                        Capture {
                            ...
                            file_capture_index: 1,
                            stanza_capture_index: 1,
                            location: Location { row: 10, column: 14 }
                        }
                        .into()
                    ),
                    name: var1.clone(),
                    location: Location { row: 10, column: 20 },
                }
                .into(),
                value: UnscopedVariable {
                    name: loc1.clone(),
                    location: Location { row: 10, column: 27 },
                }
                .into(),
                location: Location { row: 10, column: 10 },
            }
            .into(),
        ]]
    );
...
fn can_parse_inherit_directives() {
    let source = r#"
        inherit .scope
    "#;
    let file = File::from_str(tree_sitter_python::LANGUAGE.into(), source).expect("parse to succeed");
