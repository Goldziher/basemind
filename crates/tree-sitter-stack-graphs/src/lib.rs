...
//! Maintained fork of `tree-sitter-stack-graphs` (origin repo
//! <https://github.com/Goldziher/basemind>), originally derived from github/stack-graphs v0.10.0
//! and modernized (tree-sitter 0.26, edition 2024). See `ATTRIBUTIONS.md` for provenance.
//!
...
impl StackGraphLanguage {
    ...
    pub fn new(language: tree_sitter::Language, tsg: tree_sitter_graph::ast::File) -> StackGraphLanguage {
    ...
    pub fn from_str(language: tree_sitter::Language, tsg_source: &str) -> Result<StackGraphLanguage, LanguageError> {
...
impl LanguageError {
    pub fn display_pretty<'a>(&'a self, path: &'a Path, source: &'a str) -> impl std::fmt::Display + 'a {
        match self {
...
impl<'a> Builder<'a> {
    fn new(sgl: &'a StackGraphLanguage, stack_graph: &'a mut StackGraph, file: Handle<File>, source: &'a str) -> Self {
    ...
    pub fn build(
        ...
        globals.add(ROOT_NODE_VAR.into(), root_node.into()).unwrap_or_default();
...
impl tree_sitter_graph::CancellationFlag for &dyn CancellationFlag {
    fn check(&self, at: &'static str) -> Result<(), tree_sitter_graph::CancellationError> {
        CancellationFlag::check(*self, at).map_err(|err| tree_sitter_graph::CancellationError(err.0))
    }
}
    ...
    fn load(mut self, cancellation_flag: &dyn CancellationFlag) -> Result<(), BuildError> {
        ...
        for source_ref in self.graph.iter_nodes() {
            ...
            for (sink_ref, edge) in source.iter_edges() {
                ...
                self.stack_graph.add_edge(source_handle, sink_handle, precedence);
...
impl<'a> Builder<'a> {
    ...
    fn load_pop_scoped_symbol(&mut self, node_ref: GraphNodeRef) -> Result<Handle<Node>, BuildError> {
        let node = &self.graph[node_ref];
    ...
    fn load_pop_symbol(&mut self, node_ref: GraphNodeRef) -> Result<Handle<Node>, BuildError> {
        ...
        let node_handle = self.stack_graph.add_pop_symbol_node(id, symbol, is_definition).unwrap();
        if is_definition {
            self.load_definiens_info(node_ref, node_handle)?;
        }
        Ok(node_handle)
    ...
    fn load_push_scoped_symbol(&mut self, node_ref: GraphNodeRef) -> Result<Handle<Node>, BuildError> {
        let node = &self.graph[node_ref];
        let symbol = match node.attributes.get(SYMBOL_ATTR) {
            Some(symbol) => self.load_symbol(symbol)?,
            None => return Err(BuildError::MissingSymbol(node_ref)),
    ...
    fn load_push_symbol(&mut self, node_ref: GraphNodeRef) -> Result<Handle<Node>, BuildError> {
        ...
        Ok(self.stack_graph.add_push_symbol_node(id, symbol, is_reference).unwrap())
    }

    fn load_scope(&mut self, node_ref: GraphNodeRef) -> Result<Handle<Node>, BuildError> {
        let node = &self.graph[node_ref];
        let id = self.node_id_for_graph_node(node_ref);
        let is_exported = self.load_flag(node, IS_EXPORTED_ATTR)? || self.load_flag(node, IS_ENDPOINT_ATTR)?;
    ...
    fn load_flag(&self, node: &GraphNode, attribute: &str) -> Result<bool, BuildError> {
        match node.attributes.get(attribute) {
            Some(value) => value
                .as_boolean()
                .map_err(|_| BuildError::UnknownFlagType(attribute.to_string(), format!("{}", value))),
            None => Ok(false),
        }
    }

    fn load_source_info(&mut self, node_ref: GraphNodeRef, node_handle: Handle<Node>) -> Result<(), BuildError> {
        let node = &self.graph[node_ref];
    ...
    fn load_definiens_info(&mut self, node_ref: GraphNodeRef, node_handle: Handle<Node>) -> Result<(), BuildError> {
        let node = &self.graph[node_ref];
    ...
    fn load_node_debug_info(&mut self, node_ref: GraphNodeRef, node_handle: Handle<Node>) -> Result<(), BuildError> {
        let node = &self.graph[node_ref];
        for (name, value) in node.attributes.iter() {
            let name = name.to_string();
            if let Some(suffix) = name.strip_prefix(DEBUG_ATTR_PREFIX) {
                let value = match value {
                    Value::String(value) => value.clone(),
                    value => value.to_string(),
                };
                let key = self.stack_graph.add_string(suffix);
    ...
    fn load_edge_debug_info(
        ...
        for (name, value) in edge.attributes.iter() {
            ...
            if let Some(suffix) = name.strip_prefix(DEBUG_ATTR_PREFIX) {
                ...
                let key = stack_graph.add_string(suffix);
    ...
    fn verify_attributes(&self, node: &GraphNode, node_type: &str, allowed_attributes: &HashSet<&'static str>) {
...
pub trait FileAnalyzer {
    ...
    fn build_stack_graph_into(
        ...
        path: &Path,
        source: &str,
        all_paths: &mut dyn Iterator<Item = &Path>,
