...
fn can_support_injected_nodes() {
    ...
    builder.build(&globals, &NoCancellation).expect("Failed to build graph");

    check_stack_graph_nodes(&graph, file, &["[test.py(0) exported scope]", "[test.py(1) scope]"]);
    check_stack_graph_edges(&graph, &["[test.py(1) scope] -0-> [test.py(0) exported scope]"]);
}
