...
fn build_and_check_stack_graph_edges(python_source: &str, tsg_source: &str, expected_edges: &[&str]) {
    let (graph, _) = build_stack_graph(python_source, tsg_source).expect("Could not load stack graph");
...
pub(super) fn check_stack_graph_edges(graph: &StackGraph, expected_edges: &[&str]) {
    ...
    let expected_edges = expected_edges.iter().map(|s| s.to_string()).collect::<BTreeSet<_>>();
