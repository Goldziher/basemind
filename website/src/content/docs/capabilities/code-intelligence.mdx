...
confused with an import, and `goto_definition` lands on the right target — including across
...
<Aside type="note">
Resolution **refines but never shrinks** a result set. `find_callers` reports every call site
whose callee matches the name — the same sound floor `find_references` uses — and marks each
